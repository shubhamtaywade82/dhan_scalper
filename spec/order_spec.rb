...
RSpec.describe DhanScalper::Order do
  ...
  describe "#initialize" do
    it "creates an order with all attributes" do
      order = described_class.new(order_id, security_id, side, quantity, price)
    ...
    it "normalizes side to uppercase" do
      order = described_class.new(order_id, security_id, "buy", quantity, price)
      expect(order.side).to eq("BUY")
    end

    it "converts quantity to integer" do
      order = described_class.new(order_id, security_id, side, "100", price)
      expect(order.quantity).to eq(100)
    end

    it "converts price to float" do
      order = described_class.new(order_id, security_id, side, quantity, "150.50")
      expect(order.price).to eq(150.50)
    end
  ...
  describe "#buy?" do
    ...
    it "returns false for SELL orders" do
      order = described_class.new(order_id, security_id, "SELL", quantity, price)
  ...
  describe "#total_value" do
    it "calculates total value correctly" do
      order = described_class.new(order_id, security_id, side, 100, 150.50)
      expect(order.total_value).to eq(15_050.0)
    end

    it "handles zero quantity" do
      order = described_class.new(order_id, security_id, side, 0, 150.50)
      expect(order.total_value).to eq(0.0)
    end

    it "handles zero price" do
      order = described_class.new(order_id, security_id, side, 100, 0.0)
      expect(order.total_value).to eq(0.0)
    end
  ...
  describe "#to_s" do
    ...
    it "handles different sides" do
      sell_order = described_class.new(order_id, security_id, "SELL", quantity, price)
  ...
  describe "edge cases" do
    it "handles very large numbers" do
      order = described_class.new(order_id, security_id, side, 1_000_000, 999_999.99)

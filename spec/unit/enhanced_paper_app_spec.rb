...
RSpec.describe DhanScalper::PaperApp, :unit do
  ...
  describe "#analyze_and_trade" do
    ...
    it "executes trades when signals are generated" do
      expect(paper_app).to receive(:execute_trade).with("NIFTY", :bullish, 25_000.0, base_config["SYMBOLS"]["NIFTY"])
      paper_app.analyze_and_trade
    end
  ...
  describe "#get_holy_grail_signal" do
    ...
    context "when data loading fails" do
      before do
        allow(DhanScalper::CandleSeries).to receive(:load_from_dhan_intraday).and_raise(StandardError,
                                                                                        "Data loading failed")
      end
  ...
  describe "error handling" do
    ...
    context "when broker operations fail" do
      ...
      it "handles broker errors gracefully" do
        expect do
          paper_app.execute_trade("NIFTY", :bullish, 25_000.0, base_config["SYMBOLS"]["NIFTY"])
